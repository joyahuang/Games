...
public class G {
  ...
  public static G.V LEFT = new G.V(-1, 0), RIGHT = new G.V(1, 0), UP = new G.V(0,
      -1), DOWN = new G.V(0, 1);
  ...
  public static class VS {
    ...
    public boolean hit(int x, int y) {
      return (x > this.loc.x && y > this.loc.y && (x < this.loc.x + this.size.x) && (y
          < this.loc.y + this.size.y));
    }
  ...
  public static abstract class Button {
    ...
    public void show(Graphics g) {
      ...
      vs.fill(g, lnf.back);
      if (boarder) {
        vs.draw(g, enable ? lnf.boarder : lnf.disable);
      }
