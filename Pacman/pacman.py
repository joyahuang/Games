...
class Pacman(GameCharacter):
    ...
    def __init__(self, maze, game_controller):
        ...
        self.x_add = 0
        self.y_add = 0
        # need to define thie property during initialization
        self.on_left = False
        self.on_right = False
        self.on_top = False
        self.on_bottom = True
        ...
        self.WALL_TOLERANCE = 12  # make tolerance more bigger to turn easier
    ...
    def update(self):
        ...
        drawing to screen"""
        # judge which road the pacman is on
        ...
        if (self.mouth_angle >= self.max_mouth_angle):
            # if mouth angle reached max_mouth_angle, means it opens completely
            # we need to close it
            self.open_close = -(self.mouth_speed)
        if (self.mouth_angle <= self.min_mouth_angle):
            # if mouth angle reached min_mouth_angle, means it closes
            # we need to open it
            self.open_close = self.mouth_speed
